...
function renderCompleted(){
    ...
    for(let i=0; i<complete.length; i++){
        text +="<li><input type='checkbox' onchange='unchecked(this)' id='"+ complete[i] +"' checked><label>"+complete[i]+"</label><input type='text'><button class='edit' onclick='Edit(this)'>Edit</button><button class='delete' onclick='del(this)'>Delete</button></li>"
       // console.log('id is',incomplete[i]);
    }
...
function Update()
    ...
    function del(obj)
    {
     ...
     function unchecked(obj){
         let unchecked_li = obj.closest('li');
         let node = Array.from(unchecked_li.parentElement.children);
         let index = node.indexOf(unchecked_li);
         console.log(index);
